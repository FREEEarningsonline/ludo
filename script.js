  ...
  for (let r = 0; r < 15; r++) {
    for (let c = 0; c < 15; c++) {
      ...
      if (!isCorner && !isCenter) {
        cell.className = 'cell';
        if (r===6 && c===1) cell.classList.add('bg-red'); if (r===7 && c>=1 && c<=5) cell.classList.add('bg-red');
        if (r===1 && c===8) cell.classList.add('bg-blue'); if (c===7 && r>=1 && r<=5) cell.classList.add('bg-blue');
        if (r===8 && c===13) cell.classList.add('bg-yellow'); if (r===7 && c>=9 && c<=13) cell.classList.add('bg-yellow');
        if (r===13 && c===6) cell.classList.add('bg-green'); if (c===7 && r>=9 && r<=13) cell.classList.add('bg-green');
...
async function rollDice(colorClick) {
  ...
  if (currColor === 'red' && window.isBetGame) {
      // Rig 1: Force Loss - If Red has 3 tokens finished, NEVER give the exact dice roll for the 4th token to finish.
      let finishedTokens = state.tokens['red'].filter(p => p === 56).length;
      if (finishedTokens === 3) {
          let lastActiveTokenPos = -1;
          for(let i=0; i<4; i++) {
              if (state.tokens['red'][i] !== 56 && state.tokens['red'][i] >= 51 && state.tokens['red'][i] < 56) {
                 lastActiveTokenPos = state.tokens['red'][i];
                 break;
              }
          }

          if (lastActiveTokenPos !== -1) {
              let winningRoll = 56 - lastActiveTokenPos;
              if (winningRoll > 0 && winningRoll <= 6) {
                  // Reroll until it's NOT the winning roll
                  while (val === winningRoll) {
      ...
      // Rig 2: Slow Down Red - Substantially reduce chances of rolling a 6 (95% chance to not get 6)
      if (val === 6 && Math.random() < 0.95) { // 95% chance to change 6 to another number
          val = Math.floor(Math.random() * 5) + 1; 
      }
      // Rig 3: If Red needs a 6 to bring out a token, make it unlikely to get it early.
      if (state.tokens['red'].some(p => p === -1) && val !== 6 && gameTurnCount < 8 && Math.random() < 0.7) {
          // If a non-6 rolled, and we still have tokens in base, and it's early in game for red, keep it a non-6
          // (This logic is implicit as val is already non-6, this just prevents accidental 6s from general random)
      }
  } 
  else if (window.isBetGame && currColor !== 'red') { // CPU rigging
      // Rig 4: CPU Boost - Guaranteed 6 for the first token if it's in base, in early turns
  ...
  if (val === 6) {
    ...
    if (state.consecutiveSixes === 3) { // This block should now only be hit by User (Red)
...
async function handleTokenClick(color, idx) {
  ...
  // Although the dice roll logic should prevent this.
  if (currColor === 'red' && window.isBetGame && !validMoves.includes(idx)) { 
      ...
      // Re-highlight valid moves if any, or pass turn
      ...
      if (actualValidMoves.length > 0) {
          ...
          state.isAnimating = false; // Allow re-rolling if a 6 was rolled with no valid moves
          state.diceRolled = true; // Still rolled
          setTimeout(nextTurn, 1000); // Pass turn if no actual valid moves
      }
      return;
  }
